<x-layouts.authenticated :title="'Recordatorios · '.$plan->name">
    <x-page-header :title="'Recordatorios de '.$plan->name" :description="$plan->patient->full_name">
        <x-slot:actions>
            <a href="{{ route('plans.show', $plan) }}" class="inline-flex min-h-11 items-center rounded-xl border border-ink-300 px-4 text-sm font-semibold">Volver al plan</a>
        </x-slot:actions>
    </x-page-header>
    ...
    <form method="POST" action="{{ route('reminders.update', $plan) }}" class="mt-8 space-y-6">
        ...
        <x-card class="p-5 sm:p-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-ink-600">Pausar conserva todos los horarios y no pausa el plan.</p>
                </div>
                <label class="flex min-h-11 items-center gap-3 rounded-xl border border-ink-300 px-4 py-2">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" value="1" class="size-5 rounded border-ink-300 text-brand-600" @checked(old('is_active', $plan->reminderConfiguration->is_active))>
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            @foreach($days as $weekday => $day)
                ...
                <fieldset class="rounded-2xl border border-ink-200 bg-white p-5 shadow-sm">
                    <legend class="px-1 text-base font-semibold">{{ $day }}</legend>
                    <div class="mt-3 grid gap-4 sm:grid-cols-2">
                        @foreach([0, 1] as $slot)
                            <label class="block">
                                <span class="text-sm font-medium text-ink-700">Horario {{ $slot + 1 }}</span>
                                <input type="time" name="schedules[{{ $weekday }}][]" value="{{ $times[$slot] ?? '' }}" aria-label="{{ $day }}, horario {{ $slot + 1 }}" class="mt-1 min-h-11 w-full rounded-xl border border-ink-300 bg-white px-3 text-base focus:border-brand-500 focus:outline-none">
                            </label>
                        @endforeach
                    </div>
        ...
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <a href="{{ route('reminders.index') }}" class="inline-flex min-h-11 items-center justify-center rounded-xl px-5 text-sm font-semibold text-ink-700">Cancelar</a>
