<x-layouts.authenticated title="Pacientes">
    <x-page-header title="Pacientes" description="Busca, consulta y administra pacientes activos e inactivos.">
        <x-slot:actions>
            <div class="flex flex-col gap-2 sm:flex-row">
                <a href="{{ route('patients.archived') }}" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-ink-300 bg-white px-5 py-2.5 text-sm font-semibold text-ink-700 hover:bg-ink-100">Ver archivados</a>
    ...
    <form method="GET" action="{{ route('patients.index') }}" class="mt-8 grid gap-4 rounded-2xl border border-ink-200 bg-white p-4 shadow-sm sm:grid-cols-[minmax(0,1fr)_12rem_auto] sm:items-end sm:p-5">
        <div>
            <label for="search" class="block text-sm font-semibold text-ink-800">Buscar paciente</label>
            <input id="search" name="search" type="search" value="{{ $search }}" placeholder="Nombre, apellido, DNI o teléfono" class="mt-2 min-h-11 w-full rounded-xl border border-ink-300 bg-white px-3 py-2 text-ink-950 shadow-sm">
        </div>
        <div>
            <label for="status" class="block text-sm font-semibold text-ink-800">Estado</label>
            <select id="status" name="status" class="mt-2 min-h-11 w-full rounded-xl border border-ink-300 bg-white px-3 py-2 text-ink-950 shadow-sm">
        ...
        <div class="flex gap-2">
            <button type="submit" class="min-h-11 flex-1 rounded-xl bg-ink-900 px-4 py-2 text-sm font-semibold text-white hover:bg-ink-700">Aplicar</button>
            @if ($search !== '' || $status)
                <a href="{{ route('patients.index') }}" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-ink-300 px-4 py-2 text-sm font-semibold text-ink-700 hover:bg-ink-100">Limpiar</a>
            @endif
        </div>
    </form>
        ...
        @if ($patients->isEmpty())
            <x-card>
                ...
                >
                    <x-slot:action>
                        @if ($search !== '' || $status)
                            <a href="{{ route('patients.index') }}" class="inline-flex min-h-11 items-center rounded-xl border border-ink-300 px-4 py-2 text-sm font-semibold text-ink-700">Limpiar filtros</a>
                        @else
                ...
                @foreach ($patients as $patient)
                    <article class="rounded-2xl border border-ink-200 bg-white p-5 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0"><h2 class="font-semibold text-ink-950">{{ $patient->full_name }}</h2><p class="mt-1 text-sm text-ink-600">{{ $patient->whatsapp_phone }}</p></div>
                            <span @class(['rounded-full px-2.5 py-1 text-xs font-semibold', 'bg-emerald-100 text-emerald-800' => $patient->status === 'active', 'bg-ink-200 text-ink-700' => $patient->status === 'inactive'])>{{ $patient->status === 'active' ? 'Activo' : 'Inactivo' }}</span>
                        </div>
                        <p class="mt-4 text-sm text-ink-600">DNI: {{ $patient->dni ?: 'No registrado' }}</p>
