<x-layouts.authenticated :title="$patient->full_name">
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_21rem]">
        <div class="space-y-6">
            ...
            <x-card><div class="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-lg font-semibold">Planes</h2><p class="mt-1 text-sm text-slate-600">Cada plan conserva sus propias rutinas y estado.</p></div>@if($patient->status === 'active')<a href="{{ route('plans.create',['patient'=>$patient->id]) }}" class="inline-flex min-h-11 items-center rounded-xl bg-brand-600 px-4 text-sm font-semibold text-white">Nuevo plan</a>@endif</div><div class="divide-y divide-slate-200">@forelse($patient->plans as $plan)<a href="{{ route('plans.show',$plan) }}" class="flex flex-col gap-2 p-5 hover:bg-slate-50 sm:flex-row sm:items-center sm:justify-between"><span><span class="font-semibold">{{ $plan->name }}</span><span class="mt-1 block text-sm text-slate-600">{{ $plan->starts_on->format('d/m/Y') }} – {{ $plan->ends_on->format('d/m/Y') }} · {{ $plan->routines_count }} rutina(s)</span></span><span class="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">{{ ['active'=>'Activo','paused'=>'En pausa','finished'=>'Finalizado'][$plan->status] }}</span></a>@empty<div class="p-5"><x-empty-state title="Paciente sin planes" description="Crea un plan en pausa para comenzar a configurarlo."/></div>@endforelse</div></x-card>
        </div>
